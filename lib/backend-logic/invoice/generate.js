...
const fillDataTemplate = async (invoiceData, logo = null) => {
    ...
    if (logo && logo?.type) {
        ...
        html = html.replace(
            "<img/>",
            `<img src='${imageUrl}' alt='logo' class=' object-cover ' style="max-width:64px; max-height:64px; background-color:white;"/>`
        );
    } else {
        const image = fs.readFileSync(`${process.cwd()}/public/images/invoice-icon-light.png`, 'utf-8');
        const base64 = `data:image/png;base64,${image.toString("base64")}`;
        html = html.replace(
            "<img/>",
            `<img src='${base64}' alt='logo' class=' object-cover ' style="max-width:64px; max-height:64px; background-color:white;"/>`
        );
    }
